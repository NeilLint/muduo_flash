...
void GPU_RunState::deallocateGPUMemory()
{
    ...
    if (d_extraHiddenBuffer)
    {
        d_extraHiddenBuffer = nullptr;
    }
    if (d_currentActivation)
    {
        HIP_CHECK(hipFree(d_currentActivation));
        d_currentActivation = nullptr;
    }
    if (d_branchActivation)
    {
        HIP_CHECK(hipFree(d_branchActivation));
        d_branchActivation = nullptr;
    }
    if (d_extraBuffer)
    {
        HIP_CHECK(hipFree(d_extraBuffer));
        d_extraBuffer = nullptr;
    }
    if (d_hiddenBuffer_extraHiddenBuffer)
    {
        HIP_CHECK(hipFree(d_hiddenBuffer_extraHiddenBuffer));
        d_hiddenBuffer_extraHiddenBuffer = nullptr;
    }
    if (d_q)
    {
    ...
    {
        HIP_CHECK(hipFree(d_attentionScores));
        d_attentionScores = nullptr;
    }
    if (d_logits)
    {
